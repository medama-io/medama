...
@schema.parametrize(
    operation_id="get-user", data_generation_methods=[DataGenerationMethod.positive]
)  # Not possible to do negative tests on this endpoint
@settings(max_examples=EXAMPLE_COUNT)
...
@schema.auth(UserAuth)
@schema.parametrize(
    operation_id="delete-user", data_generation_methods=[DataGenerationMethod.positive]
)
@settings(max_examples=EXAMPLE_COUNT)
