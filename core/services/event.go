...
func (h *Handler) PostEventHit(ctx context.Context, req api.EventHit, params api.PostEventHitParams) (api.PostEventHitRes, error) {
	...
	// Verify hostname exists before doing anymore work.
	...
	case api.EventUnloadEventHit:
		...
		err := h.analyticsDB.AddPageDuration(ctx, event)
		if err != nil {
			log.Error().Err(err).Msg("hit: failed to add page duration")
			return ErrInternalServerError(err), nil
		}

		// Log success
		log.Debug().Msg("hit: add page duration")
	default:
		log.Error().Msg("hit: invalid event hit type")
