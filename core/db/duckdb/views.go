...
func (c *Client) AddPageView(ctx context.Context, event *model.PageView) error {
	exec := `--sql
	INSERT INTO views (bid, hostname, pathname, is_unique, referrer, title, country_code, language, ua_raw, ua_browser, ua_version, ua_os, ua_device_type, screen_width, screen_height, utm_source, utm_medium, utm_campaign, date_created)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`

	// Insert the page view into the database
	_, err := c.DB.ExecContext(ctx, exec, event.BID, event.Hostname, event.Pathname, event.IsUnique, event.Referrer, event.Title, event.CountryCode, event.Language, event.RawUserAgent, event.BrowserName, event.BrowserVersion, event.OS, event.DeviceType, event.ScreenWidth, event.ScreenHeight, event.UTMSource, event.UTMMedium, event.UTMCampaign, event.DateCreated)
	if err != nil {
		return err
	}
